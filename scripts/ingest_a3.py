...
SOURCE_FILE = REPO_ROOT / "rag_sources" / "A3_ausveg_vegnet_nsw_future_proofing.md"
...
def parse_metadata_and_content(text):
    ...
    for line in lines:
        ...
        if not in_content:
            ...
            if stripped.startswith("Project Code:"):
                metadata["project_code"] = stripped.split(":", 1)[1].strip()
                continue

            if stripped.startswith("Year Published:"):
                metadata["year_published"] = stripped.split(":", 1)[1].strip()
                continue

            if stripped.startswith("Project Provider:"):
                metadata["project_provider"] = stripped.split(":", 1)[1].strip()
                continue
...
def main():
    ...
    document_metadata = {
        ...
        "project_provider": metadata.get("project_provider")
    }
