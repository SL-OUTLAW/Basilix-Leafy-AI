...
SOURCE_FILE = REPO_ROOT / "rag_sources" / "source19_uf_ifas_water_nutrient_management.md"
...
def parse_metadata_and_content(text):
    ...
    for line in lines:
        ...
        if not in_content:
            ...
            if stripped.startswith("Original publication date:"):
                metadata["publication_date"] = stripped.split(":", 1)[1].strip()
                continue

            if stripped.startswith("Reviewed:"):
                metadata["reviewed"] = stripped.split(":", 1)[1].strip()
                continue
